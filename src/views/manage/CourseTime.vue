<template>
    ...
    <el-table :data="getYxsFilter.slice((currentPage - 1)* pagesize, currentPage * pagesize)" style="width: 100%">
        <el-table-column type="index" ></el-table-column>
        <el-table-column label="学院名称" prop="dwmc" width="220rem" ></el-table-column>
        <el-table-column label="排课总数" prop="sum" ></el-table-column>

        <el-table-column label="操作" width="300rem">
            <template #header>
                <el-input v-model="search" placeholder="输入学院名称、排课总数过滤" />
            </template>
            <template #default="scope" >
                <el-button type="primary" @click="showSysCountCourse(scope.row)">查看</el-button>
            </template>
        </el-table-column>
        ...
        <template #append>
            ...
            :total="getYxsFilter.length">    
            </el-pagination>
        </template>
     </el-table>
    
    <!-- 弹窗 -->
    <el-dialog v-model="dialogVisible" title="实验室排课统计" width="80%" :before-close="handleClose">
...
export default {
  ...
  methods: {
    ...
    // 获取所有院系所信息
    getYxsList(id) {
      this.$axios.post("/weixin-sysxk/yxscountcourse", { xnxq: id}).then(res => {
        const data = res.data
        if (data.code == 200) {
    ...
    changeXnxq() {
      ...
      Loading.show()
      this.getYxsList(this.Xnxq.xnxqh)
      Loading.hide()
    },

    showSysCountCourse(row) {
      this.Yxs = JSON.parse(JSON.stringify(row))
      this.dialogVisible = true

      // 刷新 设备列表
      Loading.show()
      this.getSysListById(this.Xnxq.xnxqh, this.Yxs.dwh)
      Loading.hide()
    },
  ...
  computed: {
      getYxsFilter() {
          return this.YxsList.filter( (data) => !this.search 
        || data.dwmc.includes(this.search.toLowerCase()) 
        || data.sum.includes(this.search.toLowerCase()))
      },
        getSysFilter() {
