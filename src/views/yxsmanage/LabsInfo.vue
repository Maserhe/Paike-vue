   ...
   <el-dialog v-model="dialogVisible" title="实验室管理" width="60%" :before-close="handleClose">
    ...
    <el-tabs @tab-click="tabsClick" v-model="manageActiveName">
      ...
      <el-tab-pane label="修改实验室信息" name="first">
        ...
        <el-form ref="sysInfoRef" :model="SysInfo" :label-position="'left'" class="sys-form" v-if="SysInfo.systype != 2">
        ...
        <el-descriptions :column="1" border v-if="SysInfo.systype == 2">
            <el-descriptions-item><template #label><el-icon> <i-tickets /></el-icon>学院</template>{{ Yxs.dwmc }}</el-descriptions-item>
            <el-descriptions-item><template #label><el-icon> <i-location-information /></el-icon>实验室门牌号</template>{{ SysInfo.sysmph }}</el-descriptions-item>
            <el-descriptions-item><template #label><el-icon> <i-timer /></el-icon>实验室类型</template> {{ SysInfo.sysmc }}</el-descriptions-item>
            <el-descriptions-item><template #label><el-icon> <i-remove-filled /></el-icon>故障</template> 该实验室有故障报修申请，不允许修改操作 </el-descriptions-item>
        </el-descriptions>
